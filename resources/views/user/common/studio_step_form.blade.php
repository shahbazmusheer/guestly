...
<form id="steps-form" method="post" action="{{route('save_studio_step_form')}}" enctype="multipart/form-data">
    @csrf
        ...
        <div class="form-slider-track">
            ...
            <div class="form-step" data-step="0">
                ...
                <div class="form-content-scrollable">
                    <div class="form-grid">
                        <div class="form-group floating-label-group"><input type="text" value="{{ old('studio_name') ? old('studio_name') : '' }}" name="studio_name" id="studio-name" required
                                                                            placeholder=" "><label for="studio-name">Studio Name</label></div>
                        <div class="form-group floating-label-group"><input type="email" name="studio_email" value="{{ old('studio_email') ? old('studio_email') : '' }}" id="business-email" required
                                                                            placeholder=" "><label for="business-email">Business Email</label></div>
                        <div class="form-group full-width floating-label-group"><input type="text" name="studio_address" value="{{ old('studio_address') ? old('studio_address') : '' }}"
...
{{--                            </select><label for="languages">Languages</label></div>--}}
                        <div class="form-group full-width floating-label-group"><input type="url" id="website-url" name="website_url" value="{{ old('website_url') ? old('website_url') : '' }}"
...
{{--                        </div>--}}
            ...
            <div class="form-step" data-step="1">
                ...
                <div class="form-content-scrollable">
                    <div class="form-group floating-label-group"><input type="number" name="stations_available" value="{{ old('stations_available') ? old('stations_available') : '1' }}" min="1" id="stations_available" required
                                                                        placeholder=" "><label for="stations_available">Total Guest Stations Available</label></div>
                    <div class="form-group floating-label-group"><input type="number" name="total_stations" value="{{ old('total_stations') ? old('total_stations') : '1' }}" min="1" id="total_stations" required
...
{{--                        </select><label>Total Guest Stations Available</label></div>--}}
                    <div class="floating-label-group"><select name="studio_type" required>
                            <option value="1" {{ old('studio_type') ? (old('studio_type')=='1' ? 'selected' : '') : 'selected' }}>Walk-In</option>
                            <option value="2" {{ old('studio_type')=='2' ? 'selected' : '' }}>Appointment</option>
                            <option value="3" {{ old('studio_type')=='3' ? 'selected' : '' }}>Private Studio</option>
                        </select><label>Studio Type</label></div>
                    <div class="field-row-bordered"><span class="row-label">Require Portfolio</span>
                        <div class="billing-toggle"><span>No</span><label class="switch"><input name="require_portfolio" {{ old('require_portfolio')=='on' ? 'checked' : '' }} type="checkbox"><span
                                    class="slider"></span></label><span>Yes</span></div>
                    </div>
                    <div class="field-row-bordered"><span class="row-label">Accept Bookings Now</span>
                        <div class="billing-toggle"><span>No</span><label class="switch"><input name="accept_bookings" {{ old('accept_bookings')=='on' ? 'checked' : '' }} type="checkbox"><span
                                    class="slider"></span></label><span>Yes</span></div>
                    </div>
                    <div class="floating-label-group"><select name="preferred_duration" required>
                            <option value="0" {{ old('preferred_duration')=='0' ? 'selected' : '' }}>No Preference</option>
                            <option value="1" {{ old('preferred_duration')=='1' ? 'selected' : '' }}>Short term (less than a month)</option>
                            <option value="2" {{ old('preferred_duration')=='2' ? 'selected' : '' }}>Long term (more than a month)</option>
                    ...
                    <div class="field-row-bordered"><span class="row-label">Allow Guest to Choose</span>
                        <div class="billing-toggle"><span>No</span><label class="switch"><input name="allow_guest_to_choose" {{ old('allow_guest_to_choose')=='on' ? 'checked' : '' }} type="checkbox"><span
                                    class="slider"></span></label><span>Yes</span></div>
                    </div>
                    <div class="form-group full-width floating-label-group"><textarea rows="3" name="bio"
                                                                                      id="bio" placeholder=" ">{{ old('bio') ? old('bio') : '' }}</textarea><label for="studio-address">Write something about your studio</label></div>

                    <div class="floating-label-group">
                        <select name="commission_type" id="commission_type" required>
                            <option value="" disabled hidden selected></option>
                            <option value="0" {{ old('commission_type')=='0' ? 'selected' : '' }}>Fixed</option>
                            <option value="1" {{ old('commission_type')=='1' ? 'selected' : '' }}>Percentage</option>
                            <option value="2" {{ old('commission_type')=='2' ? 'selected' : '' }}>Custom</option>
                        </select>
                        <label>Commission Options</label>
                    </div>

                    <div class="form-group floating-label-group" id="commission_value_group" style="{{old('commission_type') ?? 'display:none;' }}">
                        <input type="number" value="{{ old('commission_value') ? old('commission_value') : 0 }}" name="commission_value" id="commission_value" placeholder=" ">
                    ...
                    <div class="upload-policy-row">
                        ...
                        <button type="button" class="upload-link" id="upload_guest_policy_btn">Upload</button>
                    </div>
                </div>
...
{{--                    <div class="supplies-group" id="selected-supplies"></div>--}}
                            ...
                            <div class="amenities-row">
                                ...
                                <div>
                                    <label class="radio-container">
                                        <input type="checkbox" {{ old('amenities') ? (in_array($amenity->id, old('amenities')) ? 'checked' : '') : '' }} value="{{$amenity->id}}" name="amenities[]">
                ...
                <div class="button-group"><button class="nav-btn back-btn">Back</button><button
                        class="nav-btn next-btn" type="submit" id="finish-btn">Next</button></div>
            </div>
        </div>
    </div>
...
<!-- FAILED Modal (REQUIRED for the JavaScript to work) -->
<div class="modal-overlay" id="failure-modal">
    <div class="modal-content">
        <!-- Using a placeholder icon, update path if needed -->
        ...
        <div id="failure-modal-message">
            <!-- Dynamic H2 and P injected here -->
            <h2>Setup Failed</h2>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        function loadOldTags(selectId, containerId, inputName, selectedIds, selectedNames, completeData, oldData) {
            if (oldData) {
                const select = document.getElementById(selectId);
                const container = document.getElementById(containerId);
                if (select && container) {
                    oldData.forEach(optionId => {
                        let option = completeData.find(item => item.id == optionId);
                        if (option) {
                            const id = option.id;
        ...
        loadOldTags('supplies-provided', 'selected-supplies', 'supplies[]', selectedSupplyIds, selectedSupplies, @json($supplies), @json(old('supplies')));
        loadOldTags('design-specialties-provided', 'selected-design-specialties', 'design_specialties[]', selectedDesignSpecialtyIds, selectedDesignSpecialties, @json($design_specialties), @json(old('design_specialties')));
        ...
        if (modalContinueBtn) {
            modalContinueBtn.addEventListener('click', handleModalContinue);
        }
        ...
        window.addEventListener('load', () => {
            ...
            const nextStepRoute = "{{ session('next_step') ? route(session('next_step')) : '' }}";
            ...
            if (successMessage) {
                modalContinueBtn.setAttribute('data-next-route', nextStepRoute);
                modalContinueBtn.setAttribute('onclick', `window.location.href='${nextStepRoute}'`);
                mainContainer.style.filter = 'blur(5px)';
